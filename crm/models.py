...
class CustomerDistribution(models.Model):
    ...
    def __str__(self):
        return "顾问：%s,客户：%s"%(self.user.name,self.customer.name)
...
class SaleRank(models.Model):
    ...
    def __str__(self):
        return  "用户：%s,数量：%s,权重:%s"%(self.user.name,self.num,self.weight)
...
class ConsultRecord(models.Model):
    ...
    consultant = models.ForeignKey(verbose_name="跟踪人", to='UserInfo',limit_choices_to={"depart":1000})
...
class CourseRecord(models.Model):
    ...
    teacher = models.ForeignKey(verbose_name="讲师", to='UserInfo',limit_choices_to=models.Q(depart__in=[1003,1004,1005]))
